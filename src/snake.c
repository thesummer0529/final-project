...
static Map *createMap(int w,int h){
    ...
    for(int i=0;i<h;i++){ 
        m->grid[i]=(int*)malloc(w*sizeof(int)); 
        for(int j=0;j<w;j++) 
            m->grid[i][j]=EMPTY; 
    }
    return m;
...
static void updateGame(Map *map, Snake *snake, GameState *st)
{
    ...
    //計算新蛇頭座標 
    if (snake->dir == UP)          newHead.y--;
    else if (snake->dir == DOWN)   newHead.y++;
    else if (snake->dir == LEFT)   newHead.x--;
    else                           newHead.x++;

    //撞牆
    if (newHead.x < 0 || newHead.x >= map->w ||
        newHead.y < 0 || newHead.y >= map->h) {
        gameOver(st);
        return;
    }
    ...
    if (cell == FOOD) {
        ...
        int newSp = 500000 - (st->score / 10) * 100000;
        if (newSp < 200000) newSp = 200000;
        snake->speed = newSp;
    }
    ...
    for (int i = 0; i < map->h; i++)
        for (int j = 0; j < map->w; j++)
            if (map->grid[i][j] == SNAKE_HEAD ||
                map->grid[i][j] == SNAKE_BODY)
                map->grid[i][j] = EMPTY;
...
static void renderGame(Map *map,
                       Snake *snake,
                       GameState *st,
                       int eaten,
                       float elapsed)
{
    ...
    for (int i = 0; i < map->h; i++) {
        for (int j = 0; j < map->w; j++) {
            ...
            char ch = '.';
            if      (c == FOOD)           ch = 'F';
            else if (c == OBSTACLE)       ch = 'X';
            else if (c == BOOSTER_SPEED)  ch = 'S';
            else if (c == BOOSTER_SHRINK) ch = 'R';
            else if (c == BOOSTER_LIFE)   ch = 'L';
            else if (c == SNAKE_HEAD)     ch = 'H';
            else if (c == SNAKE_BODY)     ch = 'O';
            putchar(ch);
        }
        putchar('\n');
    }
...
{
    Record rec[MAX_REC];
    int    cnt = loadScores(rec, MAX_REC);
    ...
    qsort(rec, cnt, sizeof(Record), cmp);
